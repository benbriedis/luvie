...
//    addCell: Box<dyn Fn(Cell)> + 'static,
    addCell: Box<dyn Fn(Cell)>,
    modifyCell: Box<dyn Fn(usize,Cell)>,
    rightClick: Box<dyn Fn(usize)>,
...
//        let mode2 = Rc::clone(&mode1);
        ...
        Self {
            ...
            addCell: Box::new(addCell),
            modifyCell: Box::new(modifyCell),
            rightClick: Box::new(rightClick)
        }
    }
    ...
    {   
/* OPTIONS:
    1. Wrap all widget state into a separate GridState / GridSelf and pass it manually through
    2. Wrap individual fields in Rc<RefCell<>> and pass manually through`

    3. Wrap all widget state into a separate GridImpl and in most calls 'self' refers to that, not Grid
*/

        let other = Rc::new(RefCell::new(self));

//XXX appears to more of a 'setDraw'...
        other.widget.draw(move |this| {       //XXX is w = widget, = Grid, or other?
//XXX MAYBE remove this from here if widget_extends does a similar job            
            (other.draw)();
        });

//TODO do without??
        self.widget.handle(move |this, ev| {
            self.handle(ev)
        });
    }

    fn handle(self:&mut Self,event:Event) -> bool
    {
        let s = self.settings;
...
fltk::widget_extends!(Grid<'static>, Widget, widget); 
