...
impl <'a:'static,F1>GridWrap<F1>
where
    F1: Fn(usize) + 'a
{
    pub fn new(state: GridState, grid: Grid<F1>) -> Self
    {   
        let width = (state.numCols as f32 * state.colWidth).round() as i32;
        let height = (state.numRows as f32 * state.rowHeight).round() as i32;

        let mut widget = Widget::new(0,0,width,height,None); //XXX can we use Box here instead?

//        widget.draw(grid.draw);
//        widget.handle_event(grid.handle);

/* OPTIONS:
1. Use another mutex
2. Add widget these callbacks to GridStore
3. Add grid to GridStore
4. Remove self from all draw() ad handle functions
5. Add 'static back to things (NOTE though the right click handler is probably not really (only) at fault) 
6. Fold right click handler into grid (NOTE though the right click handler is probably not really (only) at fault) 
7. unsafe?
8. Convert some or all over to using messages
9. Use Observer or alternative pattern
*/
...
fltk::widget_extends!(GridWrap, Widget, widget); 

//TODO consider adding a convenience function to create widget
...
//        let color = Rc::new(RefCell::new(Color::Red));
    ...
    pub fn new(state: &GridState,rightClick: F1) -> Self
    {   
        Self {
            state,
    ...
    //app::redraw();  //XXX dont do this all the time
                        ...
                        _ => {
                            ...
                            if curX < s.colWidth * s.numCols as f32 && curY < s.rowHeight * s.numRows as f32 {
                                if let None = Self::overlappingCell(self.state.cellIterator(),&cell,None) {
                                    //state.cache.clear();  
                                    self.state.addCell(cell);
                                    app::redraw();  //TODO check required
                                }
                            }
                        }
                        ...
                        CursorMode::MOVING(data) => {
                            ...
                            if data.amOverlapping {
                               // shell.publish(GridAreaMessage::Cells(CellMessage::ModifyCell(data.cellIndex,data.lastValid)));
                                self.state.modifyCell(data.cellIndex,data.lastValid);
                            }
                            else {
                                //shell.publish(GridAreaMessage::Cells(CellMessage::ModifyCell(data.cellIndex,data.workingCell)));
                                self.state.modifyCell(data.cellIndex,data.workingCell);
                            }
...
*/
    ...
    //FIXME For moves consider snapping on end if we picked it up closer to the end that the start (or having a mode)       
        ...
        data.amOverlapping = Self::overlappingCell(self.state.cellIterator(),&cell,Some(data.cellIndex)).is_some();
    ...
    */
    ...
    //XXX if changing grid size want the num of pixels to remain constant.
        ...
        match data.side {
            Side::LEFT => {
                ...
                let neighbour = Self::overlappingCell(self.state.cellIterator(),&testCell,Some(data.cellIndex));
                let min = if let Some(n) = neighbour { self.state.getCell(n).col + self.state.getCells(n).length } else { 0.0 };
                if cell.col < min {
                    cell.col = min;
                }
            ...
            Side::RIGHT => {
                ...
                let neighbour = Self::overlappingCell(self.state.cellIterator(),&cell,Some(data.cellIndex));
                let max = if let Some(n) = neighbour { self.state.getCell(n).col } else { s.numCols as f32 };
    ...
    fn overlappingCell(cellIter:impl Iterator<Item = Cell>,a:&Cell,selected:Option<usize>) -> Option<usize>
    {
        let aStart = a.col;
        let aEnd = a.col + a.length;

        for (i,b) in cellIter.enumerate() {
