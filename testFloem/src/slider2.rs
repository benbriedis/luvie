...
/* --- Ben hacks --- */
use floem::{
    Renderer, event::EventPropagation, id::ViewId, keyboard::{Key, NamedKey,KeyState,KeyboardEvent},pointer::{PointerButtonEvent, PointerEvent}, prop, prop_extractor, style:: {BorderRadiusProp, CustomStylable, CustomStyle, Foreground, Height, Style 
    }, style_class, unit::{Pct, PxPct, PxPctAuto}, view::View, views::Decorators
...
use style::{BorderRadiusProp, CustomStyle};
use unit::Pct;
use crate::{
    Renderer,
...
fn border_radius(style: &BarStyle, size: f64) -> RoundedRectRadii {
    ...
    RoundedRectRadii {
        top_left: floem::view::border_radius(
            border_radius.top_left.unwrap_or(PxPct::Px(0.0)),
            size,
        ),
        top_right: floem::view::border_radius(
            border_radius.top_right.unwrap_or(PxPct::Px(0.0)),
            size,
        ),
        bottom_left: floem::view::border_radius(
            border_radius.bottom_left.unwrap_or(PxPct::Px(0.0)),
            size,
        ),
        bottom_right: floem::view::border_radius(
